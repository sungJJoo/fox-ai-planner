...
원래는 구글 시트 하나로 관리했는데, 폭스러닝센터 AI 연구소를 운영하면서 아래 불편들이 쌓였습니다. 그걸 하나씩 풀려고 만든 도구입니다.

| 겪던 문제 | 이 도구의 해결 |
|---|---|
| **시트 UI가 불편** — 내용이 많아지면 스크롤을 한참 내려야 하고, 핸드폰으로 보기 너무 불편 | 한 화면에 정리된 대시보드 + 모바일 반응형 + PWA(앱처럼 설치) |
| **3주마다 메인 연구원이 바뀜** — 매번 누구 차례인지 손으로 계산 | 앵커 날짜 기준 **3주 순환 자동 계산**, 오늘 담당자 자동 강조 |
| **업무가 많아지면 중요한 걸 놓침** — 다른 일 하다 보면 원래 업무를 까먹음 | 마감 **카운트다운 배지 · 누락 업무 알림 · 당일 마감 브라우저 알림**으로 안 놓치게 |
| **행사·외부 미팅이 잦음** (센터 특성) — 기록하고 한눈에 볼 곳이 필요 | **연간 캘린더**(행사·공휴일·계획) + 한국 공휴일 자동 표시 |
| **소규모 팀이라 누가 쉬는지 바로 알아야 함** — 연차·휴무 공유가 중요 | 근무일정의 **연차·반차 자동 추출** + 캘린더 자동 표시, 7일 이내 강조 |
| **나중에 인사고과·회고 때 "내가 뭘 했지?"** | 완료 업무가 **'완료 업무' 시트에 자동 아카이빙** → 담당자·기간별로 기록 축적 |
